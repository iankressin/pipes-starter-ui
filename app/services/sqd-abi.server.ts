...
import { address as toSvmAddress, type Address } from '@solana/addresses'
...
import type { NetworkType } from '~/starter/types'

// Server-side port of the CLI implementation:
// pipes-sdk/packages/cli/src/services/sqd-abi.ts
...
function getEvmChainId(slug: string): string {
  const networks = cliNetworks as unknown as { evm: { slug: string; chainId?: string }[] }
  return (
    networks.evm.find((n) => n.slug === slug)?.chainId ||
    (() => {
...
class EvmAbiService extends AbiService {
  // CLI default is local proxy. Optionally override with `SQD_EVM_ABI_PROXY_URL`.
  private static SERVICE_URL = process.env.SQD_EVM_ABI_PROXY_URL ?? 'http://127.0.0.1:3004/evm/abi'
...
class SvmAbiService extends AbiService {
  ...
  override generateTypes(projectPath: string, contractAddresses: string[]): void {
    ...
    for (const address of contractAddresses) {
      execSync(`npx --yes @subsquid/solana-typegen@latest ${outputDir} ${address}`, { stdio: 'ignore' })
    }
  }
  ...
  private async fetchSvmContractData(address: string) {
    ...
    return {
      ...
      contractEvents: idl.instructions.map((i: SvmInstruction) => ({
        ...
        inputs: [
          ...i.args
            .map<RawAbiItem>(({ name, type }) => ({ name: toCamelCase(name), type }))
            .filter((a) => typeof a.type !== 'object'),
          ...i.accounts.map<RawAbiItem>(({ name }) => ({ name: toCamelCase(name), type: 'publicKey' })),
        ],
      })),
    }
  }
